...
class MysqliDb {
    ...
    public function join(string|object $joinTable, string $joinCondition, string $joinType = ''): MysqliDb {
        ...
        if (!is_object($joinTable)) {
            $joinTable = $this->prefix . filter_var($joinTable, FILTER_SANITIZE_FULL_SPECIAL_CHARS);
        }
    ...
    public function prepare(string $query, array $bindParams = null, bool $sanitize = true): array {
        ...
        $this->_query = $query;
        if ($sanitize) {
            $this->_query = filter_var($query, FILTER_SANITIZE_FULL_SPECIAL_CHARS, FILTER_FLAG_NO_ENCODE_QUOTES);
        }
        $stmt = $this->_prepareQuery();
